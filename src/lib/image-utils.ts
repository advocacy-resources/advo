 ...
 */
export function bufferToDataUrl(buffer: Buffer, mimeType: string = 'image/jpeg') {
...
export function processResourceImages(resource: any) {
  ...
  if (resource.profilePhoto) {
    ...
    if (typeof resource.profilePhoto === 'string' && 
        (resource.profilePhoto.startsWith('data:') || resource.profilePhoto.startsWith('/uploads/'))) {
  ...
  if (resource.bannerImage) {
    // If it's already a data URL or a file path, keep it as is
    if (typeof resource.bannerImage === 'string' && 
        (resource.bannerImage.startsWith('data:') || resource.bannerImage.startsWith('/uploads/'))) {

...
const ReviewList = ({ resourceId, refreshTrigger = 0 }: ReviewListProps) => {
  ...
  const submitEditReview = async () => {
    ...
    try {
      const response = await fetch(`/api/v1/resources/${resourceId}/reviews/${editingReview.id}`, {
        method: "PUT",
        headers: {
  ...
  const confirmDeleteReview = async () => {
    ...
    try {
      const response = await fetch(`/api/v1/resources/${resourceId}/reviews/${reviewToDelete}`, {
        method: "DELETE",
      });
  ...
  if (isLoading) {
    return <div className="text-center py-4 text-sm md:text-base">Loading reviews...</div>;
  }

  if (error) {
    return <div className="text-red-500 py-4 text-center text-sm md:text-base">{error}</div>;
  }

  if (reviews.length === 0) {
    return <div className="text-center py-4 text-gray-400 text-sm md:text-base">No reviews yet. Be the first to share your experience!</div>;
  }
      ...
      {reviews.map((review) => (
        <div key={review.id} className="bg-gray-900 p-4 rounded-md">
          <div className="flex flex-col sm:flex-row justify-between items-center sm:items-start mb-3 sm:mb-2">
            <h4 className="font-medium text-center sm:text-left">{review.User?.name || "Anonymous User"}</h4>
            <span className="text-sm text-gray-400 mt-1 sm:mt-0">
              {new Date(review.createdAt).toLocaleDateString()}
            </span>
          </div>
          <p className="text-gray-200 text-center sm:text-left">{review.content}</p>
